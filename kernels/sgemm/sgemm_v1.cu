...
__global__ void Sgemm( 
    ...
    // 水平方向为 x, 竖直方向为 y
    // (bx, by) refers to a block in output matrix C
    int bx = blockIdx.x;
    int by = blockIdx.y;

    // (tx, ty) refers to a mini block in block (bx, by)
    // Thread index
    int tx = threadIdx.x;
    int ty = threadIdx.y;
    ...
    const int ldg_num_a = BLOCK_SIZE_M * BLOCK_SIZE_K / (THREAD_NUM_PER_BLOCK * 4); // 对于矩阵A中的block 每个 thread 向量化加载数据的个数
    const int ldg_num_b = BLOCK_SIZE_K * BLOCK_SIZE_N / (THREAD_NUM_PER_BLOCK * 4); // 对于矩阵B中的block 每个 thread 向量化加载数据的个数
    float ldg_a_reg[4*ldg_num_a];
    float ldg_b_reg[4*ldg_num_b];

    // threads number in one row
    const int A_TILE_THREAD_PER_ROW = BLOCK_SIZE_K / 4; // 矩阵A中的一个block, 一行需要多少个thread进行读取
    const int B_TILE_THREAD_PER_ROW = BLOCK_SIZE_N / 4; // 矩阵B中的一个block, 一行需要多少个thread进行读取
    ...
    for (int i = 0 ; i < BLOCK_SIZE_M ; i += A_TILE_ROW_STRIDE) {
        int ldg_index = i / A_TILE_ROW_STRIDE * 4; // 第几次读取
...
int main(int argc, char** argv) {
    ...
    // const int BLOCK_SIZE_K = atoi(argv[4]);
